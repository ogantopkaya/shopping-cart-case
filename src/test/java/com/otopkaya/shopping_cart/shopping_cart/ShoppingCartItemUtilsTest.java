...
public class ShoppingCartItemUtilsTest {
    ...
    @Test
    public void testGroupedByCategory(){
        Category cat1 = new Category("cat1", null);
        Category cat2 = new Category("cat2", cat1);
        Category cat3 = new Category("cat3", cat1);
        ...
        HashMap<Category, List<ShoppingCartItem>> group = ShoppingCartItemUtils.groupedByCategory(Arrays.asList(item1, item2, item3, item4));

        Assert.assertEquals(group.size(), 3);

        Assert.assertArrayEquals(
                group.get(cat1).toArray(),
                Arrays.asList(item1, item2).toArray()
        );

        Assert.assertArrayEquals(
                group.get(cat2).toArray(),
                Arrays.asList(item3).toArray()
        );

        Assert.assertArrayEquals(
                group.get(cat3).toArray(),
                Arrays.asList(item4).toArray()
        );
    }
